...
func Run(args []string, s usm.Storage) {
...
// Cmd wraps the methods for a usm cli command
type Cmd interface {
	...
	Run(s usm.Storage) error   // Run runs the command
}

// Usage prints the command usage
func Usage(commands []Cmd) {
	template := `usm cli is the CLI application for USM

Usage: usm cli <command> [arguments]

The commands are:

{{ range $k, $cmd := . }}	{{ printf "%-13s %s\n" $cmd.Name $cmd.Description }}{{ end }}
Use "usm cli <command> -help" for more information about a command.
`
...
// func askNumber(prompt string, min, max int) (int, error) { return 0, nil }
// func getInstallPath() (string, error) { return "", nil }
// it will use a session from the USM_SESSION env variable if set,
...
func askPasswordMode(prompt string, options []usm.PasswordMode, defaultMode usm.PasswordMode) (usm.PasswordMode, error) {
	var defaultIdx int
...
func parseItemPath(path string, opts itemPathOptions) (itemPath, error) {
	...
	for i, v := range parts {
		...
		case 0:
			ip.vaultName = v
		case 1:
			var itemType usm.ItemType
			var err error
			if opts.wildcard && v == "*" {
				break
			}
			itemType, err = usm.ItemTypeFromString(v)
			if err != nil {
				return ip, err
			}
...
func loadVaultKey(s usm.Storage, vaultName string) (*usm.Key, error) {
	sessionID := os.Getenv(usm.ENV_SESSION)
	if sessionID != "" {
...
func loadVaultKeyWithSession(s usm.Storage, vaultName string, sessionID string) (*usm.Key, error) {
