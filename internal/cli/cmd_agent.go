...
func (cmd *AgentCmd) Description() string {
	return "Manages the USM agent"
}

// Usage displays the command usage
func (cmd *AgentCmd) Usage() {
	template := `Usage: usm cli agent COMMAND
...
func (cmd *AgentCmd) Run(s usm.Storage) error {
	switch cmd.command {
	...
	case agentSessionsSubCmd:
		...
		if _, err := fmt.Fprintln(w, "Session ID\tVault\tLifetime"); err != nil {
			return err
		}
