...
func TestStorageOSDeleteVault(t *testing.T) {
	name := "delete-me"
	password := "secret"
	...
	_, err = storage.CreateVaultKey(name, password)
	require.NoError(t, err)
	key, err := storage.LoadVaultKey(name, password)
	require.NoError(t, err)
	_, err = storage.CreateVault(name, key)
	require.NoError(t, err)

	// Create a sample item to ensure directory is populated
	v, err := storage.LoadVault(name, key)
	require.NoError(t, err)
	note := NewNote()
	note.Name = "some"
	require.NoError(t, v.AddItem(note))
	require.NoError(t, storage.StoreItem(v, note))

	// Delete vault
	require.NoError(t, storage.DeleteVault(name))
