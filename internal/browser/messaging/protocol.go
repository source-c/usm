...
// mux is a native messaging multiplexer for USM
...
func (pm *mux) Handle(w io.Writer, r io.Reader) error {
	...
	res := &Response{}
	h.Serve(res, req)
	if err := writeNativeMessage(w, res); err != nil {
		return err
	}
	return res.Error
}
...
func writeNativeMessage(w io.Writer, v any) error {
	...
	responseLen := len(encodedResponse)
	if responseLen > 0xFFFFFFFF {
		return fmt.Errorf("response too large: %d bytes", responseLen)
	}
	err = binary.Write(w, binary.LittleEndian, uint32(responseLen))
	if err != nil {
		return err
	}
