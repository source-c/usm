...
func (a *activityRenderer) scaleDot(dot *canvas.Circle, off float32) {
	...
	alphaFloat := int(float32(a.maxCol.A) * off)
	if alphaFloat < 0 {
		alphaFloat = 0
	} else if alphaFloat > 255 {
		alphaFloat = 255
	}
	alpha := uint8(alphaFloat) //nolint:gosec // bounds checked above
...
func (a *activityRenderer) updateColor() {
	...
	// Convert uint32 to uint8 with bounds checking
	r := uint8((rr >> 8) & 0xFF)    //nolint:gosec // bit masking ensures safe conversion
	g := uint8((gg >> 8) & 0xFF)    //nolint:gosec // bit masking ensures safe conversion
	b := uint8((bb >> 8) & 0xFF)    //nolint:gosec // bit masking ensures safe conversion
	aVal := uint8((aa >> 8) & 0xFF) //nolint:gosec // bit masking ensures safe conversion
	a.maxCol = color.NRGBA{R: r, G: g, B: b, A: aVal}
}
