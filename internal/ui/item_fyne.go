...
import (
	...
	usmwidget "apps.z7.ai/usm/internal/widget"
)

// FyneItemWidget wraps all methods allow to handle a usm.Item as Fyne Widget
type FyneItemWidget interface {
	...
	Edit(ctx context.Context, key *usm.Key, w fyne.Window) fyne.CanvasObject
	// Item returns a deep copy of the embedded usm item
	// It will panic if the copy fails
	Item() usm.Item
	// OnSubmit performs the necessary actions to update the item with the latest data
	// and returns a deep copy of the embedded usm item
	OnSubmit() (usm.Item, error)
}

// FynePasswordGenerator wraps all methods to show a Fyne dialog to generate passwords
type FynePasswordGenerator interface {
	ShowPasswordGenerator(bind binding.String, password *usm.Password, w fyne.Window)
}

func NewFyneItemWidget(item usm.Item, preferences *usm.Preferences) FyneItemWidget {
	switch item.GetMetadata().Type {
	case usm.NoteItemType:
		return NewNoteWidget(item.(*usm.Note))
	case usm.LoginItemType:
		return NewLoginWidget(item.(*usm.Login), preferences)
	case usm.PasswordItemType:
		return NewPasswordWidget(item.(*usm.Password), preferences)
	case usm.SSHKeyItemType:
		return NewSSHWidget(item.(*usm.SSHKey), preferences)
	}
...
func deepCopyItem(src, dst usm.Item) error {
...
func rowWithAction(label string, text string, opts rowActionOptions, w fyne.Window) []fyne.CanvasObject {
	...
	case "password":
		v = usmwidget.NewPasswordRevealer(text)
	case "url":
