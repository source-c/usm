...
func NewSSHWidget(item *usm.SSHKey, preferences *usm.Preferences) FyneItemWidget {
...
func (iw *sshItemWidget) OnSubmit() (usm.Item, error) {
...
func (iw *sshItemWidget) Item() usm.Item {
	copy := usm.NewSSHKey()
...
func (iw *sshItemWidget) Edit(ctx context.Context, key *usm.Key, w fyne.Window) fyne.CanvasObject {
	...
	privateKeyActionMenu := []*fyne.MenuItem{
		{
			...
			Action: func() {
				...
				_ = privateKeyEntryBind.Set(string(sk.MarshalPrivateKey()))
				_ = publicKeyEntryBind.Set(string(sk.MarshalPublicKey()))
				_ = fingerprintEntryBind.Set(string(sk.Fingerprint()))
			},
		},
		{
			Label: "Copy",
					...
					if err != nil {
						...
						a := dialog.NewForm("Private Key is password protected", "Confirm", "Cancel",
							...
							func(isConfirm bool) {
								...
								_ = passphraseBind.Set(passphrase)
								_ = privateKeyEntryBind.Set(string(sk.MarshalPrivateKey()))
								_ = publicKeyEntryBind.Set(string(sk.MarshalPublicKey()))
								_ = fingerprintEntryBind.Set(string(sk.Fingerprint()))
							},
