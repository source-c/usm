...
import (
	...
	"apps.z7.ai/usm/internal/favicon"
...
func NewLoginWidget(item *usm.Login, preferences *usm.Preferences) FyneItemWidget {
...
// loginItemWidget handle a usm.Login as Fyne Widget
...
func (iw *loginItemWidget) OnSubmit() (usm.Item, error) {
	...
	if iw.item.Metadata.Autofill == nil {
		iw.item.Metadata.Autofill = &usm.Autofill{}
	}
...
func (iw *loginItemWidget) Item() usm.Item {
	copy := usm.NewLogin()
...
func (iw *loginItemWidget) Edit(ctx context.Context, key *usm.Key, w fyne.Window) fyne.CanvasObject {
	...
	urlEntry.FaviconListener = func(favicon *usm.Favicon) {
...
type urlEntry struct {
	...
	FaviconListener func(*usm.Favicon)
	ctx             context.Context
	loginURL        *usm.LoginURL // keep track of the initial value before editing
	preferences     usm.FaviconDownloaderPreferences
	validationError error
}

func newURLEntryWithData(ctx context.Context, loginURL *usm.LoginURL, preferences usm.FaviconDownloaderPreferences) *urlEntry {
	e := &urlEntry{
		ctx:         ctx,
		loginURL:    loginURL,
		preferences: preferences,
...
func (e *urlEntry) FocusLost() {
	...
	go func() {
		...
		fav = usm.NewFavicon(newHostname, b, format)
		e.FaviconListener(fav)
	}()
}
