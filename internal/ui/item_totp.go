...
import (
	"context"
	"crypto/sha1" //nolint:gosec // SHA-1 required by TOTP RFC 6238 standard
	"fmt"
	"strconv"
	"time"

	"apps.z7.ai/usm/internal/otp"
...
func (t *TOTP) Edit(ctx context.Context, w fyne.Window) (fyne.CanvasObject, *usm.TOTP) {
	totp := &usm.TOTP{}
	*totp = *t.TOTP

	if totp == nil || (*totp == usm.TOTP{}) {
		totp = usm.NewDefaultTOTP()
	}
